import { createTRPCRouter, authedProcedure } from "@/server/init";import { candidates, jobRoles, events, evidence, recruiterActions } from "../schema";
import { z } from "zod";
...
export const recruiterRouter = createTRPCRouter({
  ...
  getActiveEvent: authedProcedure
    .query(async ({ ctx }) => {
  ...
  getDashboardStats: authedProcedure
    .query(async ({ ctx }) => {
      ...
      return {
        ...
        projectedHires: allRoles.reduce((sum, r) => sum + (r.targetHires ?? 0), 0),
        roles: allRoles,
      };
    }),
  ...
  getActions: authedProcedure
    .query(async ({ ctx }) => {
      return ctx.secureDb!.rls((tx) =>
        tx.select().from(recruiterActions).orderBy(desc(recruiterActions.createdAt))
      );
    }),
  ...
  createAction: authedProcedure
    .input(z.object({
      ...
      actionType: z.enum(["sync_to_ats", "follow_up_email", "schedule_interview", "move_stage"]),
      notes: z.string().optional(),
    }))
    .mutation(async ({ ctx, input }) => {
      const [action] = await ctx.secureDb!.rls((tx) =>
        tx.insert(recruiterActions)
          .values({
            userId: ctx.user.id,
