...
const STAGE_MOVE_STEPS: Record<string, { label: string; detail: string }[]> = {
  screen: [
    {
      label: "Evaluating fit score",
      detail: "Comparing against role benchmarks",
    },
    {
      label: "Flagging risk indicators",
      detail: "Scanning resume for inconsistencies",
    },
    {
      label: "Generating screening checklist",
      detail: "Tailoring questions to candidate strengths",
    },
    { label: "Moving to Screening", detail: "Pipeline stage updated" },
  ],
  interview: [
    {
      label: "Reviewing screening results",
      detail: "Validating communication and depth scores",
    },
    {
      label: "Checking calendar availability",
      detail: "Matching open interviewer windows",
    },
    {
      label: "Generating interview prep notes",
      detail: "Highlighting skills to probe",
    },
    { label: "Moving to Interview", detail: "Pipeline stage updated" },
  ],
  offer: [
    {
      label: "Analyzing interview performance",
      detail: "Reviewing panel feedback signals",
    },
    {
      label: "Benchmarking compensation",
      detail: "Aligning offer to role tier",
    },
    {
      label: "Preparing offer package",
      detail: "Drafting offer letter and terms",
    },
    { label: "Moving to Offer", detail: "Pipeline stage updated" },
  ],
  day1: [
    {
      label: "Confirming offer acceptance",
      detail: "Validating signed documents",
    },
    {
      label: "Provisioning system access",
      detail: "Setting up accounts and tools",
    },
    {
      label: "Notifying onboarding team",
      detail: "Sending first-day logistics",
    },
    { label: "Welcome aboard!", detail: "Day 1 preparations complete" },
  ],
  fair: [
    {
      label: "Resetting pipeline stage",
      detail: "Moving back to active queue",
    },
    {
      label: "Restoring recruiter visibility",
      detail: "Re-adding to fair board",
    },
...
const ACTION_STEPS: Record<string, { label: string; sub: string }[]> = {
  ...
  schedule_interview: [
    {
      label: "Scanning interviewer calendars",
      sub: "Finding available windows",
    },
    {
      label: "Checking candidate availability",
      sub: "Cross-referencing time zones",
    },
  ...
  follow_up_email: [
    {
      ...
      sub: "Personalizing message tone",
    },
  ...
  send_rejection: [
    {
      label: "Preparing rejection notice",
      sub: "Crafting professional message",
    },
  ...
  send_offer_email: [
    {
      label: "Generating offer letter",
      sub: "Pulling compensation package data",
    },
    {
      label: "Attaching offer documents",
      sub: "Terms, start date, and benefits",
    },
      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={i} className="flex items-start gap-3">
            ...
            <div
              className={`transition-opacity duration-300 ${active ? "opacity-100" : done ? "opacity-60" : "opacity-25"}`}
            >
              <p className="text-[13px] font-semibold text-[#111827]">
                {s.label}
              </p>
              {active && (
                <p className="text-[11px] text-[#6b7280] mt-0.5 animate-pulse">
                  {s.detail}
                </p>
              )}
            </div>
...
export default function CandidateDetailPage() {
  ...
  const invalidateAll = () => {
    ...
    queryClient.invalidateQueries({
      queryKey: trpc.recruiter.getActionsByCandidate.queryKey({
        candidateId: id,
      }),
    });
  };
  ...
  const handleStageMove = (stage: string) => {
    ...
    updateStage.mutate({
      id,
      stage: stage as "fair" | "screen" | "interview" | "offer" | "day1",
    });
  };
  ...
  const currentStageIndex = STAGE_ORDER.indexOf(
    displayStage as (typeof STAGE_ORDER)[number],
  );
    ...
    >
        ...
        <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-red-600 mt-1.5">
              Use the actions panel on the right to run a Social Screen or send
              a rejection.
            </p>
          </div>
          <button
      ...
      <div className="flex gap-5">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <p className="text-xs text-red-700 leading-relaxed">
                        Score of <span className="font-bold">{score}</span> is
                        below the 70-point threshold.
                        {score < 60
                          ? " Well below minimum bar for this role."
                ...
                <div className="bg-card border border-border rounded-xl shadow-sm p-5">
                  ...
                  <div className="space-y-2">
                    {strengths.slice(0, 2).map((s, i) => (
                      <div
                        key={i}
                        className="text-sm text-primary italic px-3 py-2 bg-primary/5 rounded-lg border border-primary/10"
                      >
                        "{s}"
                      </div>
                    ))}
            ...
            <div className="space-y-3">
              {actions && actions.length > 0 ? (
                actions.map((action: any) => (
                  <div
                    key={action.id}
                    className="bg-card border border-border rounded-xl shadow-sm p-4 flex items-center justify-between"
                  >
                    <div>
        ...
        <motion.div
          ...
          className="w-[220px] shrink-0 space-y-4"
        >
          {/* Actions */}
          <div className="bg-card border border-border rounded-xl shadow-sm p-4 space-y-2">
            ...
            <motion.button
              ...
              style={{
                background: "linear-gradient(135deg, #0e3d27 0%, #1f6b43 100%)",
                color: "#fff",
              }}
            >
            ...
            >
              <span className="text-sm font-semibold text-[#15803d]">
                Get Notified on Agent
              </span>
              <span className="text-[10px] text-[#6b7280]">
                powered by GPT 5.4 Codex · OpenClaw
              </span>
            </motion.button>
            ...
            {/* Action processing banner */}
            {pendingActionType && (
              <ActionProcessingBanner actionType={pendingActionType} />
            )}
            ...
            <div className="relative">
              ...
              >
                <span className="flex items-center gap-2">
                  <RefreshCw
                    className={`w-3.5 h-3.5 ${updateStage.isPending ? "animate-spin" : ""}`}
                  />
                  Move Stage
                </span>
          ...
          <div className="bg-card border border-border rounded-xl shadow-sm p-4 space-y-2">
            ...
            <button
              onClick={() =>
                fireAction(
                  "send_rejection",
                  "Rejection email queued for approval",
                )
              }
              disabled={createAction.isPending}
            ...
            <button
              onClick={() =>
                fireAction(
                  "send_offer_email",
                  "Offer email queued for approval",
                )
              }
              disabled={createAction.isPending}
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <p
                ...
                style={{
                  color: candidate.riskLevel === "high" ? "#991b1b" : undefined,
                }}
              >
                Identity & Risk Scan
              </p>
              {candidate.riskLevel === "high" && (
                <span className="ml-auto text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-red-100 text-red-700">
                  FLAGGED
                </span>
              )}
            </div>
            ...
            >
              <div>
                <p className="text-[13px] font-bold text-white">
                  Internal OpenClaw Workflow Runner
                </p>
                <p className="text-[11px] text-[#9ca3af]">
                  who Syncs Up with you once Agent starts to Screen
                </p>
              </div>
              <button
                onClick={() => setOpenClawOpen(false)}
